...
use crate::config::{HttpConfig, InstallerConfig, StorageConfig, StorageMode};
...
impl InstallerProvider {
    ...
    fn select_release<'a>(&self, releases: &'a [Release], tag: &str) -> Option<&'a Release> {
        ...
        releases.iter().find(|release| {
            ...
            let has_installer = release
                .assets
                .iter()
                .any(|asset| asset.name.starts_with("duckcoding-cli-installer-"));
            has_checksums && has_installer
        })
    }
    ...
    pub async fn sync_version(&self, version: &str) -> Result<()> {
        ...
        info!("Syncing installer version: {}", version);
    ...
    pub async fn sync_all(&self) -> Result<Vec<String>> {
        ...
        for tag in &self.config.tags {
            match self.sync_tag(tag).await {
                ...
                Ok(None) => {}
                Err(e) => {
                    warn!("Failed to sync installer tag {}: {:?}", tag, e);
            ...
            if let Some(version_meta) = provider.versions.get(version) {
                for (platform, meta) in &version_meta.platforms {
                    platforms.insert(
                        ...
                        serde_json::json!({
                            "version": version,
                            "url": format!("/installer/{}/{}/{}", version, platform, meta.filename),
                            "sha256": meta.sha256,
                            "size": meta.size
                        }),
                    );
