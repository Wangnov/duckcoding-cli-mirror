...
impl NodeProvider {
    ...
    fn select_release<'a>(&self, releases: &'a [Release], tag: &str) -> Option<&'a Release> {
        ...
        releases.iter().find(|release| {
            ...
            let has_node_payload = release
                .assets
                .iter()
                .any(|asset| asset.name.starts_with("node-v"));
            let has_shasums = release
                .assets
                .iter()
                .any(|asset| asset.name == "SHASUMS256.txt");
            has_checksums && (has_node_payload || has_shasums)
        })
    }
    ...
    pub async fn sync_version(&self, version: &str) -> Result<()> {
                    ...
                    if remote_mode {
                        ...
                        if result.sha256 != task.sha256 {
                            warn!(
                                ...
                                version, task.platform, task.sha256, result.sha256
                            );
                            match storage_mode {
                                StorageMode::Oss => {
        ...
        self.persist_shasums(version, &release).await?;
    ...
    async fn persist_shasums(&self, version: &str, release: &Release) -> Result<()> {
        ...
        let bytes = self
            .download_asset_bytes(&asset.browser_download_url)
            .await?;
        let key = self
            .cache
        ...
        match self.storage.mode {
            StorageMode::Local => {
                let path = self
                    .cache
                    .get_file_path(PROVIDER_NAME, &["versions", version, "SHASUMS256.txt"])
                    .ok_or_else(|| MirrorError::VersionNotFound(version.to_string()))?;
                if let Some(parent) = path.parent() {
                    tokio::fs::create_dir_all(parent).await?;
                }
