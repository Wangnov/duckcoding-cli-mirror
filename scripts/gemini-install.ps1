...
$Version = ""
$NodeTag = if ($env:NODE_TAG) { $env:NODE_TAG } else { "latest" }
$NodeVersion = ""
$NodePtyTag = if ($env:NODE_PTY_TAG) { $env:NODE_PTY_TAG } else { "latest" }
$NodePtyVersion = ""
$Upgrade = $false
$CheckOnly = $false
$NoModifyPath = $false
...
for ($i = 0; $i -lt $ArgsList.Count; $i++) {
    switch ($ArgsList[$i]) {
        ...
        "--node-tag" { $NodeTag = $ArgsList[++$i] }
        "--node-version" { $NodeVersion = $ArgsList[++$i] }
        "--node-pty-tag" { $NodePtyTag = $ArgsList[++$i] }
        "--node-pty-version" { $NodePtyVersion = $ArgsList[++$i] }
...
try {
    ...
    $installerArgs = @("--mirror-url", $MirrorUrl, "--install-dir", $InstallDir, "gemini", "--tag", $Tag)
    if ($Version) { $installerArgs += @("--version", $Version) }
    if ($NodeTag) { $installerArgs += @("--node-tag", $NodeTag) }
    if ($NodeVersion) { $installerArgs += @("--node-version", $NodeVersion) }
    if ($NodePtyTag) { $installerArgs += @("--node-pty-tag", $NodePtyTag) }
    if ($NodePtyVersion) { $installerArgs += @("--node-pty-version", $NodePtyVersion) }
